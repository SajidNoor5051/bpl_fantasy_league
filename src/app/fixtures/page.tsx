...
export default function FixturesPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true)
        const queryParams = selectedGameweek ? `?gameweekId=${selectedGameweek}` : ''
        ...
        } else if (!selectedGameweek && data.gameweeks && data.gameweeks.length > 0) {
  ...
  const openFixtureModal = async (fixture: Fixture) => {
    ...
    try {
      const response = await fetch(`/api/fixtures?fixtureId=${fixture.fixture_id}`)
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'LIVE':
        return <span className="bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-full">LIVE</span>
      case 'COMPLETED':
        return <span className="bg-gray-600 text-white text-xs font-semibold px-2 py-1 rounded-full">FT</span>
      case 'SCHEDULED':
        return <span className="bg-green-500 text-white text-xs font-semibold px-2 py-1 rounded-full">UPCOMING</span>
      case 'POSTPONED':
        return <span className="bg-yellow-500 text-white text-xs font-semibold px-2 py-1 rounded-full">POSTPONED</span>
      case 'CANCELLED':
        return <span className="bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded-full">CANCELLED</span>
      default:
        return null
    }
  }
  ...
  const selectedGameweekName = gameweeks.find(gw => gw.gameweek_id === selectedGameweek)?.name || 'All Gameweeks'
  const groupedFixtures = groupFixturesByDate(fixtures)
  const dateKeys = Object.keys(groupedFixtures).sort((a, b) => new Date(a).getTime() - new Date(b).getTime())
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            {gameweeks.map((gameweek) => (
              <button 
                ...
                className={`block w-full text-left px-4 py-2 hover:bg-gray-700 ${
                  selectedGameweek === gameweek.gameweek_id ? 'bg-gray-700 text-white' : 'text-gray-300'
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-700 text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">No Fixtures Found</h3>
          <p className="text-gray-400">
        ...
        <div className="space-y-8">
          {dateKeys.map(dateKey => (
            <div key={dateKey} className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 overflow-hidden">
                      ...
                      <div className="col-span-3 text-right">
                        <div className="flex items-center justify-end">
                          <div className="text-right mr-3">
                            <div className="font-medium text-white">{fixture.home_team_id.team_name}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-span-3 text-left">
                        <div className="flex items-center">
                          <div className="text-left ml-3">
                            <div className="font-medium text-white">{fixture.away_team_id.team_name}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <>
                  ...
                  <div className="mb-6">
                    <div className="text-xs text-gray-400 mb-2">
                      {fixtureDetails.fixture?.gameweeks.name} • {formatDate(selectedFixture.match_date)}
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="text-lg font-semibold text-white">{selectedFixture.home_team_id.team_name}</div>
                      <div className="px-4 py-2 bg-gray-700 rounded-md text-xl font-bold text-white">
                        {selectedFixture.home_team_score} - {selectedFixture.away_team_score}
                      </div>
                      <div className="text-lg font-semibold text-white">{selectedFixture.away_team_id.team_name}</div>
                    </div>
                          ...
                          <ul className="space-y-2">
                            ...
                            {[
                              ...
                              ...fixtureDetails.goals
                                .filter(goal => goal.team_id.team_id === selectedFixture.home_team_id.team_id)
                                .map(goal => ({
                                  type: 'goal',
                                  minute: goal.minute,
                                  data: goal
                              ...
                              ...fixtureDetails.yellowCards
                                .filter(card => card.team_id.team_id === selectedFixture.home_team_id.team_id)
                                .map(card => ({
                                  type: 'yellow',
                                  minute: card.minute,
                                  data: card
                              ...
                              ...fixtureDetails.ownGoals
                                .filter(og => og.team_id.team_id === selectedFixture.home_team_id.team_id)
                                .map(og => ({
                                  type: 'own_goal',
                                  minute: og.minute,
                                  data: og
                            ...
                            .map((event, index) => {
                              ...
                              const assist = event.type === 'goal' ? 
                                fixtureDetails.assists.find(
                                  a => a.assisted_to.player_id === event.data.player_id.player_id && 
                                      Math.abs(a.minute - event.data.minute) <= 1
                                ) : null;
                                
                              return (
                                <li key={`${event.type}-${index}`} className="p-2 bg-gray-700/40 rounded-md">
                                  <div className="flex items-center text-sm">
                                    ...
                                    <div>
                                      ...
                                      <span className="font-medium text-white">
                                        {event.data.player_id.first_name} {event.data.player_id.last_name}
                                      </span>
                                      
                                      {event.type === 'goal' && (event.data as Goal).is_penalty && (
                                        <span className="text-yellow-400 ml-1">(P)</span>
                                      )}
                                      {event.type === 'own_goal' && (
                                        <span className="text-red-400 ml-1">(OG)</span>
                                      )}
                                      {event.type === 'red' && !(event.data as RedCard).is_straight_red && (
                                        <span className="text-xs text-gray-400 ml-1">(Second Yellow)</span>
                                      )}
                                      ...
                                      {assist && (
                                        <div className="text-xs text-gray-400 mt-0.5 flex items-center">
                                          ...
                                          {assist.player_id.first_name} {assist.player_id.last_name}
                                        </div>
                                      )}
                                    </div>
                                  </div>
                            ...
                            {fixtureDetails.goals.filter(g => g.team_id.team_id === selectedFixture.home_team_id.team_id).length === 0 &&
                             fixtureDetails.yellowCards.filter(c => c.team_id.team_id === selectedFixture.home_team_id.team_id).length === 0 &&
                             fixtureDetails.redCards.filter(c => c.team_id.team_id === selectedFixture.home_team_id.team_id).length === 0 &&
                             fixtureDetails.ownGoals.filter(og => og.team_id.team_id === selectedFixture.home_team_id.team_id).length === 0 && (
                              <li className="text-sm text-gray-400 italic p-2">  </li>
                            )}
                          </ul>
                        </div>
                        ...
                        <div className="w-full">
                          ...
                          <ul className="space-y-2">
                            ...
                            {[
                              // Add goals
                              ...fixtureDetails.goals
                                .filter(goal => goal.team_id.team_id === selectedFixture.away_team_id.team_id)
                                .map(goal => ({
                                  type: 'goal',
                                  minute: goal.minute,
                                  data: goal
                              ...
                              // Add yellow cards
                              ...fixtureDetails.yellowCards
                                .filter(card => card.team_id.team_id === selectedFixture.away_team_id.team_id)
                                .map(card => ({
                                  type: 'yellow',
                                  minute: card.minute,
                                  data: card
                              ...
                              // Add own goals
                              ...fixtureDetails.ownGoals
                                .filter(og => og.team_id.team_id === selectedFixture.away_team_id.team_id)
                                .map(og => ({
                                  type: 'own_goal',
                                  minute: og.minute,
                                  data: og
                            ...
                            {/* Only show "   " if no events*/}
                            {fixtureDetails.goals.filter(g => g.team_id.team_id === selectedFixture.away_team_id.team_id).length === 0 &&
                             fixtureDetails.yellowCards.filter(c => c.team_id.team_id === selectedFixture.away_team_id.team_id).length === 0 &&
                             fixtureDetails.redCards.filter(c => c.team_id.team_id === selectedFixture.away_team_id.team_id).length === 0 &&
                             fixtureDetails.ownGoals.filter(og => og.team_id.team_id === selectedFixture.away_team_id.team_id).length === 0 && (
                              <li className="text-sm text-gray-400 italic p-2">  </li>
                            )}
                          </ul>
                        </div>
