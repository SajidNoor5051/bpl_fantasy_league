...
-- Show players and points for team_id = 21, gameweek 3 with captain logic
WITH team_players AS (
  -- 1. Get fantasy_team_players for team_id = 21 and gameweek_id = 4
  SELECT 
    ftp.fantasy_team_id,
    ftp.player_id,
    ftp.gameweek_id,
  ...
  WHERE ftp.gameweek_id = 4 
  AND ft.fantasy_team_id = 2
),
completed_fixtures AS (
  -- 2. Get completed fixtures for gameweek 4 
  SELECT 
    f.fixture_id,
    f.gameweek_id
  FROM fixtures f
  WHERE f.status = 'COMPLETED' AND f.gameweek_id = 4
),
player_stats AS (
  -- 3. Get player stats from player_match_stats for these players for those fixtures
  SELECT 
...
-- Show the results for team_id = 21
SELECT 
  pgt.player_id,
  p.first_name,
  p.last_name,
  ...
  END as final_points
FROM player_gameweek_totals pgt
JOIN players p ON pgt.player_id = p.player_id
LEFT JOIN captain_logic cl ON pgt.fantasy_team_id = cl.fantasy_team_id AND pgt.gameweek_id = cl.gameweek_id
